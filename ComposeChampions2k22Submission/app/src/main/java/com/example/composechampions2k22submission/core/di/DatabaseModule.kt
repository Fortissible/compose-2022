package com.example.composechampions2k22submission.core.di

import android.content.Context
import androidx.room.Room
import com.example.composechampions2k22submission.core.data.source.local.room.AnimeDao
import com.example.composechampions2k22submission.core.data.source.local.room.AnimeDatabase
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.android.qualifiers.ApplicationContext
import dagger.hilt.components.SingletonComponent
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
class DatabaseModule {

    @Singleton
    @Provides
    fun provideMovieDatabase(@ApplicationContext context: Context) : AnimeDatabase
    = Room.databaseBuilder(context, AnimeDatabase::class.java, "Anime.db").build()

    @Provides
    fun provideMovieDao(animeDatabase: AnimeDatabase):AnimeDao
    = animeDatabase.animeDao()
}